# -*- coding: utf-8 -*-
"""
Created on Sun May 10 20:19:11 2020
...
## step1 初始化聚类中心
    ...
    while (clusterChanged):
        ...
        numChanged = 0
        clusterAssment_buffer = copy.copy(clusterAssment[:,0])#这句话是不是非常耗时？
...
## step4 循环结束后更新聚类中心
        ...
        for j in range(k):
            ...
            comp = np.nonzero(clusterAssment[:,0] == j)[0]
            comp1 = np.nonzero(clusterAssment_buffer == j)[0]
            ...
            if(len(comp) == len(comp1) and len(comp) != 0):
                if((comp == comp1).all()):
                    ...
                    if(k-1 != j):	
                        ...
                        clusterAssment[comp_k,0] = [j] * len(comp_k)
        ...
        #print("每次质心更新用时"+str(record_end-record_start)+"秒")
#            for ii in range(len(pointsInCluster)):
#                SSE += np.power(euclDistance(centroids[j,:],pointsInCluster[ii]),2)
#        
...
if __name__=="__main__":
    ...
    start=time.time()
    ## load data
    #dataset = pd.read_csv('kdd_pre_final_resample.csv',sep=',')
	...
	#					   'variable15','variable16','variable17','variable18','variable19','variable20','variable21',\
	#					   'variable22','variable23','variable24']]
